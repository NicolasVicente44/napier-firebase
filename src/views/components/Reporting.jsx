      ...
      try {
        ...
        fetchedData.forEach((item) => {
          ...
          if (itemDate >= oneYearAgo) {
            ...
            // Collect cost data
            if (item.towingCost)
              towingCosts.push(parseFloat(item.towingCost) || 0);
            if (item.storageCosts)
              storageCosts.push(parseFloat(item.storageCosts) || 0);
            if (item.bailiffCosts)
              bailiffCosts.push(parseFloat(item.bailiffCosts) || 0);
            if (item.daysOfStorage)
              totalDaysOfStorage += parseFloat(item.daysOfStorage) || 0;
          }
        });
        ...
        const maxNoisItem = monthsWithNois.reduce(
          (max, item) => (item.nois > max.nois ? item : max),
          { nois: 0 }
        );
        const minNoisItem = monthsWithNois.reduce(
          (min, item) => (item.nois < min.nois ? item : min),
          { nois: Infinity }
        );
        ...
        const averageNois =
          monthsWithNois.length > 0 ? totalNois / monthsWithNois.length : 0;
        ...
        const variance =
          processedData.reduce(
            (acc, cur) => acc + Math.pow(cur.nois - meanNois, 2),
            0
          ) / totalPoints;
        const stdDevNois = Math.sqrt(variance);

        // Calculate total costs
        const totalTowingCosts = towingCosts.reduce(
          (acc, cost) => acc + cost,
          0
        );
        const totalStorageCosts = storageCosts.reduce(
          (acc, cost) => acc + cost,
          0
        );
        const totalBailiffCosts = bailiffCosts.reduce(
          (acc, cost) => acc + cost,
          0
        );

        // Calculate average days of storage
        const averageDaysOfStorage =
          monthsWithNois.length > 0
            ? totalDaysOfStorage / monthsWithNois.length
            : 0;
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
          <div className="container mx-auto px-6 py-8">
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
              <h1 className="text-3xl font-bold mb-6">Reporting/Analysis</h1>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg flex items-center">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold mb-2">
                      Maximum NOIS in a Month
                    </h3>
                    <p className="text-2xl font-bold">
                      {maxNoisMonth} ({maxNois})
                    </p>
                  </div>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-lg flex items-center">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold mb-2">
                      Minimum Non-Zero NOIS in a Month
                    </h3>
                    <p className="text-2xl font-bold">
                      {minNoisMonth} ({minNois})
                    </p>
                  </div>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-lg flex items-center">
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg flex items-center">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold mb-2">
                      Average Days of Storage
                    </h3>
                    <p className="text-2xl font-bold">
                      {averageDaysOfStorage.toFixed(2)}
                    </p>
                  </div>
                </div>
              </div>
